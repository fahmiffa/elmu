...
@section('content')
@section('content')
<div class="flex flex-col bg-white rounded-lg shadow-md p-6" x-data="dataTablePay({{ json_encode($lay) }})">
    <div x-init="
                ...
                <table class="min-w-full bg-white border border-gray-200 text-sm">
                    ...
                    <tbody>
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-1">
                                        <form x-show="row.status != 1 && row.reg.murid.users.fcm !== null"
                                            method="post"
                                            :action="'/dashboard/send/' + md5Component(row.id) + '/lay'">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <div x-show="modal.activeModal === row.id" x-cloak x-transition
                                            ...
                                            <div @click.away="modal.closeModal()" class="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
                                                ...
                                                <form method="POST"
                                                    :action="'/dashboard/pembayaran/' + md5Component(row.id) + '/lay'">
                                                    @csrf
                                                    <div class="mb-4">
                                                        <label
                        ...
                        <tr x-show="filteredData().length === 0">
                            <td colspan="8" class="text-center px-4 py-4 text-gray-500">Data tidak ditemukan.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
