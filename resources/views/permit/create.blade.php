...
<style>
    ...
    .ts-wrapper.single .ts-control {
        @apply px-3 py-2 border rounded-lg focus-within:border-orange-500 bg-white;
        cursor: text;
    }
...
<div class="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
    <div class="mb-4 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-800">Tambah Permit</h2>
        <a href="{{ route('dashboard.presensi.index') }}" class="text-gray-500 hover:text-gray-700">Kembali</a>
    </div>

    <form action="{{ route('dashboard.presensi.store') }}" method="POST" id="form-presensi">
        @csrf
            ...
            <select name="unit_schedules_id" id="unit_schedules_id" required
                ...
                @foreach($unitSchedules as $sch)
                    <option value="{{ $sch->id }}"
                            ...
                            {{ old('unit_schedules_id') == $sch->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <textarea name="why" rows="4" required
                      class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-orange-500">{{ old('why') }}</textarea>
...
<script>
    ...
    // Restore info siswa jika ada old value (setelah validasi backend gagal)
    @if(old('student_id'))
    onStudentChange('{{ old("student_id") }}');
