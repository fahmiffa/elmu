...
@section('content')
    <div class="flex flex-col bg-white rounded-lg shadow-md p-6">
        ...
        @isset($items)
            <form method="POST" action="{{ route('dashboard.master.grade.update', ['grade'=>$items->id]) }}" class="grid grid-cols-1">
                @method('PUT')
            @else
                <form method="POST" action="{{ route('dashboard.master.grade.store') }}" class="grid grid-cols-1">
                @endisset
                @csrf
                <div class="mb-4">
