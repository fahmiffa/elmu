...
@section('content')
<div class="flex flex-col bg-white rounded-lg shadow-md p-6" x-data="formHandler('{{ route('dashboard.master.grade.index') }}')">
    <div class="font-semibold mb-3 text-xl">{{ $action }}</div>
    @isset($items)
    <form method="POST" action="{{ route('dashboard.master.grade.update', ['grade'=>$items->id]) }}" class="grid grid-cols-1" @submit.prevent="submit">
        @method('PUT')
        @else
        <form method="POST" action="{{ route('dashboard.master.grade.store') }}" class="grid grid-cols-1" @submit.prevent="submit">
            @endisset
            @csrf
            <div class="mb-4">
