...
@section('content')
    <div class="flex flex-col bg-white rounded-lg shadow-md p-6">
        ...
        @isset($items)
            <form method="POST" action="{{ route('dashboard.master.payment.update', $items->id) }}"
                class="grid grid-cols-1">
                @method('PUT')
            @else
                <form method="POST" action="{{ route('dashboard.master.payment.store') }}">
                @endisset
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            <input type="number" min="1" name="per" value="{{ old('per', $items->month ?? '') }}"
                                class="border border-gray-300  ring-0 rounded-xl px-3 py-2 w-full focus:outline-[#FF9966]">
                        </div>
                        @error('per')
                            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
