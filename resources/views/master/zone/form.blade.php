...
@section('content')
<div class="flex flex-col bg-white rounded-lg shadow-md p-6" x-data="formHandler('{{ route('dashboard.master.zone.index') }}')">
    <div class="font-semibold mb-3 text-xl">{{ $action }}</div>
    @isset($items)
    <form method="POST" action="{{ route('dashboard.master.zone.update', $items->id) }}" @submit.prevent="submit">
        @method('PUT')
        @else
        <form method="POST" action="{{ route('dashboard.master.zone.store') }}" @submit.prevent="submit">
            @endisset
            @csrf
