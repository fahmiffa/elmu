@extends('base.layout')
@section('title', 'Ubah Password ' . $user->name)
@section('content')
<div class="flex flex-col w-full md:w-1/2 bg-white rounded-lg shadow-md p-6">
    <div class="font-semibold mb-3 text-xl">Ubah Password User: {{ $user->name }}</div>
    <form method="POST" action="{{ route('dashboard.master.user.pass.update', md5($user->id)) }}">
        @csrf
        @method('PUT')
